    ...
    angular.module('angularTreeview', []).directive('treeModel', ['$compile', function($compile) {
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                //Mai edit @ 20170625 
                // add comment text
                var nodeComment = attrs.nodeComment || 'comment';
                ...
                var template =
                    ...
                    '<span data-ng-class="node.selected" data-ng-click="' + treeId + '.selectNodeLabel(node)">{{node.' + nodeLabel + '}} {{node.' + nodeComment + '}}</span>' +
                    '<div data-ng-hide="node.collapsed" data-tree-id="' + treeId + '" data-tree-model="node.' + nodeChildren + '" data-node-id=' + nodeId + ' data-node-label=' + nodeLabel + ' data-node-children=' + nodeChildren + ' data-node-comment=' + nodeComment + '></div>' +
                    '</li>' +
                    '</ul>';
